%  Save NIFTI dataset. Support *.nii, *.nii.gz and *.hdr/*.img file extension.
...
function save_nii_gzip(nii, fileprefix, old_RGB)
   ...
   %   
   doGZ = false;
   if strfind(fileprefix,'.nii.gz')
        fileprefix = strrep(fileprefix,'.gz','');
        doGZ = true;
   end 
